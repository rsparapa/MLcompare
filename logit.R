...
##     df <- cbind(df, x.train)
##     post[[i]] <- glm(y~., family = 'binomial', data = df)
## }
## saveRDS(post, 'RDS/logit.rds')

post=readRDS('RDS/logit.rds')
...
##                   predictor = valid[[i]])
## }
## saveRDS(check, 'RDS/valid-logit.rds')

check <- readRDS('RDS/valid-logit.rds')
...
saveRDS(AUC, 'RDS/breast-logit.rds')
...
saveRDS(auc, 'RDS/auc-logit.rds')

library(pROC)
check <- list()
for(i in 1:4) check[[i]] <- roc(response = y.test[[i]], 
                  predictor = pred[[i]])
saveRDS(check, 'RDS/roc-logit.rds')

AUC <- matrix(nrow = 200, ncol = 4)
for(j in 1:200) {
    bs <- readRDS(paste0('bs/', j, '.rds'))
    for(i in 1:4) {
        pred[[i]] <- predict(post[[i]], bs$x.test) 
